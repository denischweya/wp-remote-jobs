...
export default function Edit() {
	...
	useEffect( () => {
		...
		apiFetch( { path: '/wp/v2/job_category' } ).then( ( terms ) =>
			setCategories(
				terms.map( ( term ) => ( {
					label: term.name,
					value: term.id,
		...
		apiFetch( { path: '/wp/v2/job_skills' } ).then( ( terms ) =>
			setSkills(
				terms.map( ( term ) => ( {
					label: term.name,
					value: term.id,
		...
		apiFetch( { path: '/wp/v2/job_location' } ).then( ( terms ) =>
			setLocations(
				terms.map( ( term ) => ( {
					label: term.name,
					value: term.id,
		...
		apiFetch( { path: '/wp/v2/employment_type' } ).then( ( terms ) =>
			setEmploymentTypes(
				terms.map( ( term ) => ( {
					label: term.name,
					value: term.id,
		...
		apiFetch( { path: '/wp/v2/benefits' } ).then( ( terms ) =>
			setBenefitOptions(
				terms.map( ( term ) => ( {
					label: term.name,
					value: term.id,
		...
		apiFetch( { path: '/wp/v2/salary_range' } ).then( ( terms ) =>
			setSalaryRanges(
				terms.map( ( term ) => ( {
					label: term.name,
					value: term.id,
		...
		apiFetch( { path: '/wp-remote-jobs/v1/countries' } ).then( ( data ) => {
			const countryOptions = Object.entries( data ).map(
			...
			<form onSubmit={ handleSubmit }>
				...
				<SelectControl
					...
					onChange={ ( selectedSkills ) =>
						setJobSkills(
							selectedSkills.filter( ( skill ) => skill !== '' )
						)
					}
				/>
				<RadioControl
				...
				{ isWorldwide === 'no' && (
					<Select
						...
						value={ countries.find(
							( country ) => country.value === jobLocation
						) }
						onChange={ ( selectedOption ) =>
							setJobLocation( selectedOption.value )
						}
